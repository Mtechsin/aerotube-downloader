...
class _YoutubeLoginScreenState extends State<YoutubeLoginScreen> {
  ...
  Future<void> _exportWindowsCookiesForYtdlp() async {
    ...
    try {
      // HttpOnly cookies (SID, HSID, SSID, LOGIN_INFO) cannot be read via
      // JavaScript document.cookie. Fall back to Edge browser cookies which
      // can access them through native APIs.
      ...
      LoggingService().error(
        'Failed to set up Windows cookie export: $e',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to configure cookie export'),
            duration: Duration(seconds: 3),
          ),
        );
      }
